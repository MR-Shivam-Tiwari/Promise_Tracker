...
function Task() {
  ...
  const filterTasksByStatus = (tasks, status) => tasks.filter(task => task.status === status || (status === 'To Do' && !task.status));
  ...
  const fetchTasks = async () => {
    try {
      ...
      setTaskGroups([...new Set(filteredTasks.map(task => task?.taskGroup?.groupName))]);
    } catch (error) {
