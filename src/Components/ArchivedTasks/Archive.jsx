...
function Archive() {
    ...
    return (
        <div>
            ...
            ) : (archiveTasks && Array.isArray(archiveTasks) && archiveTasks.length > 0 ? (
                    <div className="w-full max-w-8xl mx-auto">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {archiveTasks.map((task, index) => (
                                <div key={index} className="bg-white shadow-md cursor-pointer min:h-[38vh] rounded-lg p-4">
                                    <div className="flex flex-wrap gap-2 items-center justify-between mb-4">
                                        <div className='flex font-bold items-center gap-3 text-black text-sm'>
                                            <div className='flex'>
                                                <svg fill="#808080" width="20px" height="20px" viewBox="-3 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                                                    <title>group</title>
                                    ...
                                    <h3 className="text-lg font-semibold text-gray-900">{truncateText(task?.taskName, 15)}</h3>
                                    <p className="text-sm text-gray-500 mb-4">{truncateText(task?.description, 100)}</p>
                                    <div className="flex items-center justify-between text-[13px] font-bold text-gray-800 mb-2">
                                        <span>Date: {task?.startDate ? formatDate(task.startDate) : 'N/A'} - {task?.endDate ? formatDate(task.endDate) : 'N/A'}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
