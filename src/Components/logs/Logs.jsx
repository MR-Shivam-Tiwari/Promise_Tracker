    ...
    switch (log.action) {
      ...
      case 'approved':
          ...
          <>
            <div className='mb-2 mt-2'>
              <p className='text-gray-600'>
                ...
                <span className='font-bold text-orange-500 capitalize'> "{log?.details?.subTaskName}"</span> on {moment(log?.timestamp).format('DD-MM-YYYY')}
              </p>
              <span className='font-bold text-md ml-4'>Assigned To :</span>
              <span className='font-bold text-gray-600'> {
        ...
        return (
          <>
            <div className='text-gray-600 mt-2 mb-2'>
              <span className='font-bold text-blue-500'>"{log?.userId?.name}"</span> <span className='text-gray-500 '>Add a comment<span className='font-semibold text-orange-500'>{ ` "${log?.details?.text}"`}</span> </span>  on {moment(log?.timestamp).format('DD-MM-YYYY')}
            </div>
            <hr />
          </>
        );
          ...
          <>
            <div className='mb-2 mt-2'>
              <p className='text-gray-600'>
                ...
                <span className='font-bold text-orange-500 capitalize'> "{log?.details?.subTaskName}"</span> status to <span className='font-bold text-green-500'>Done</span> on {moment(log?.timestamp).format('DD-MM-YYYY')}
              </p>
        ...
        return (
          <>
            <div className='mb-2 mt-2'>
              <p className='text-gray-600'>
                <span className='font-bold text-blue-500'>"{log?.userId?.name}"</span> change subtask
                <span className='font-bold text-orange-500 capitalize'> "{log?.details?.subTaskName}"</span> status to <span className='font-bold text-red-500'>Pending</span> on {moment(log?.timestamp).format('DD-MM-YYYY')}
              </p>
